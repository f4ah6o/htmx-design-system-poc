...
import { CATALOG_COMPONENTS, CATALOG_TEMPLATE_MAP, STATUS_META, USE_CASE_TASKS, findTask, type Task } from './data';
import { Layout, IndexPage, renderTaskPanel } from './templates';
  ...
  return c.html(
    <Layout title="Atoms - htmx Design System">
      <div class="max-w-6xl mx-auto px-4 py-12">
        ...
        <div class="space-y-6">
          {CATALOG_COMPONENTS.filter(c => c.category === 'Atoms').map(comp => (
  ...
  return c.html(
    <Layout title="Molecules - htmx Design System">
      <div class="max-w-6xl mx-auto px-4 py-12">
        ...
        <div class="space-y-6">
          {CATALOG_COMPONENTS.filter(c => c.category === 'Molecules').map(comp => (
            <div class="p-6 bg-white rounded-lg shadow-sm">
              <h2 class="text-xl font-semibold mb-2">{comp.title}</h2>
              <p class="text-gray-600">{comp.summary}</p>
            </div>
  ...
  return c.html(
    <Layout title="Organisms - htmx Design System">
      <div class="max-w-6xl mx-auto px-4 py-12">
        ...
        <div class="space-y-6">
          {CATALOG_COMPONENTS.filter(c => c.category === 'Organisms').map(comp => (
            <div class="p-6 bg-white rounded-lg shadow-sm">
              <h2 class="text-xl font-semibold mb-2">{comp.title}</h2>
              <p class="text-gray-600">{comp.summary}</p>
            </div>
...
app.get('/catalog', (c) => {
  const defaultSlug = CATALOG_COMPONENTS[0]?.slug || '';
  return c.html(
      ...
      <div class="max-w-7xl mx-auto px-4 py-8">
        ...
        <div class="grid md:grid-cols-3 gap-6">
          ...
          <div class="md:col-span-2">
            <div id="catalog-detail" class="p-8 bg-white rounded-lg shadow-sm border border-gray-200">
    ...
    <Layout title="Use Cases">
      <div class="max-w-6xl mx-auto px-4 py-8">
        ...
        <div class="mb-8 p-6 bg-white rounded-lg shadow-sm">
          ...
          <form hx-post="/api/use-cases/tasks" hx-target="#task-panel" hx-swap="outerHTML">
            <div class="space-y-4">
              <input
                type="text"
                name="title"
...
app.post('/api/validate/username', async (c) => {
  ...
  if (username.length < 3) {
    return c.html('<p class="text-red-600">✗ ユーザー名は3文字以上である必要があります</p>');
  }

  if (username.length > 20) {
    return c.html('<p class="text-red-600">✗ ユーザー名は20文字以下である必要があります</p>');
  }

  const reserved = ['admin', 'root', 'system'];
  if (reserved.includes(username.toLowerCase())) {
    return c.html('<p class="text-red-600">✗ このユーザー名は使用できません</p>');
  }
...
app.get('/api/catalog/:component_slug', (c) => {
  ...
  if (!component) {
    return c.html('<div class="p-6 text-red-600">コンポーネントが見つかりません</div>', 404);
  }
...
app.post('/api/use-cases/tasks', async (c) => {
  ...
  const summary = formData.get('summary')?.toString().trim() || '詳細は後で追記します';

  if (!title) {
    return c.html(renderTaskPanel('タスク名を入力してください', 'error', USE_CASE_TASKS, STATUS_META), 400);
  }
  ...
  return c.html(renderTaskPanel('新しいタスクを追加しました', 'success', USE_CASE_TASKS, STATUS_META));
});
...
app.delete('/api/use-cases/tasks/:task_id', (c) => {
  ...
  if (!task) {
    return c.html(renderTaskPanel('指定したタスクが見つかりません', 'error', USE_CASE_TASKS, STATUS_META), 404);
  }
  ...
  return c.html(renderTaskPanel('タスクを削除しました', 'info', USE_CASE_TASKS, STATUS_META));
});
...
app.get('/api/use-cases/tasks/:task_id/detail', (c) => {
  ...
  if (!task) {
    return c.html('<div class="text-red-600">タスクが見つかりません</div>', 404);
  }
